...
class EdgeLoss(nn.Module):
    def __init__(self, kernel_size=3):
        ...
        self.padding = (kernel_size - 1) // 2  # 保证padding后尺寸不变

    def forward(self, pred_mask, target_mask):
        """
        pred_mask: 模型输出的概率图 (经过Sigmoid), 形状 [B, 1, H, W]
        target_mask: 目标二元掩码, 形状 [B, 1, H, W]
        """
        # 生成目标边缘掩码
        target_dilated = F.max_pool2d(
            target_mask, kernel_size=self.kernel_size,
            stride=1, padding=self.padding
        )
        target_edge = torch.abs(target_dilated - target_mask)

        # 计算二元交叉熵损失
        edge_loss = F.binary_cross_entropy(pred_mask, target_edge)
